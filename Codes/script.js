...
function matchWhite(str) {
    for (let i = 0; i < white_pieces.length; i++) {
        if (str.includes(white_pieces[i])) return 1;
    }
    return 2;
}
...
function whatPiece(str) {
    if (matchWhite(str) == 1) {
        for (let i = 0; i < white_pieces.length; i++) {
            if (str.includes(white_pieces[i])) return piece_sym_white[i];
        }
    } else {
        for (let j = 0; j < white_pieces.length; j++) {
            if (str.includes(black_pieces[j])) return piece_sym_black[j];
        }
    }
}
